...
class CallingController extends Controller
{
	...
	if($user){
        ...
        if(($user->calling->where('status','Waiting')->count()>0) || ($user->is_checkout)){
		...
		//return response()->json(['status'=>'error2','message' => 'Test '],201);
/*
                if($sqlsvr_userinfo){
                    $data=[
 ...
 */
		...
		//---- add into mysql database attandance --------------
                    $data=[
                        ...
                        'date' =>  date('Y-m-d H:i'),
			'status' => 'Present',
			'remark' => (($user->is_checkout)?'selfout':'')
                    ];   
		...
		//	return response()->json(['status'=>'success','message'=>count($tokenkey)]);
		...
		*/
		...
		if($user->is_checkout)
			 return response()->json(['status'=>'success_selfout','message' => 'Student Name: ' . $user->name . ' is self-checkout.','student_id' => $user->email],201);
		else
    ...
    public function getwaitingvoice(Request $request){
        if($request->ajax()){
            ...
            $path = secure_asset("https://storage.googleapis.com/ics_school/audio");
        ...
        //orderBy('new_at')->get()
            ...
            ->addColumn('thumnail', function(Dismissal $dismissal){
                ...
                if(Storage::exists('/photo/' . $dismissal->student->photo) && !empty($dismissal->student->photo))
                       $html .="<img src='" . "https://storage.googleapis.com/ics_school/photo/" . $dismissal->student->photo . "' style='width:100%;height:auto; ' class='p-0 m-0'>";
                else
                        $html .="<img src='" . "https://storage.googleapis.com/ics_school/image/student-avatar.png" . "' style='width:100%;height:auto' class='p-0 m-0'>";
