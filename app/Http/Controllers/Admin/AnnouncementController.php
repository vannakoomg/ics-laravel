...
class AnnouncementController extends Controller
{
    ...
    public function store(Request $request){
       ...
       // dd($request->all());
		...
		$ImageUpload->resize(1920, 1920, function ($constraint) {
                        $constraint->aspectRatio();
                });
		$ImageUpload->save($ImagePath . $fileName);

		$ImagePath = 'storage/thumbnail/';
		//$ImageUpload->resize(250,125);
		$ImageUpload->resize(400, 400, function ($constraint) {
                   $constraint->aspectRatio();
                });
		$ImageUpload = $ImageUpload->save($ImagePath . $fileName);
        ...
        public function update(Request $request,Announcement $announcement){
           ...
           // return $request->all();
		    ...
		    Storage::delete(['thumbnail/' . $announcement->thumbnail]);
		}
		$ImageUpload=Image::make($files);
                $ImagePath = 'storage/image/';
                $fileName =  "image-".time().'.'.$request->imgupload->getClientOriginalExtension();
		$ImageUpload->resize(1024, 1024, function ($constraint) {
                        $constraint->aspectRatio();
                });
		$ImageUpload->save($ImagePath . $fileName);
