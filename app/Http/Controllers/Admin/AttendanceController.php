...
class AttendanceController extends Controller
{
    ...
    $dateRange = [
        ...
        $date1->endOfMonth()->setTime(23, 59, 59)->format('Y-m-d')
    ];
   // dd($dateRange);
    ...
    public function store(Request $request){
	    ...
	    $data=[];
		    ...
		    $data =  [
                    ...
                    'student_id' => $student,
                    'date' => ($Attendance?$Attendance->date:$date),
		...
		if($Attendance)
	            $Attendance->update($data);
                    ...
                    Attendance::create($data);   // $Attendance->update($data);
            ...
            foreach($request->class_id as $k=>$class_id){
                ...
                if(is_array($students_presents) && $students_presents[0]!=""){
                    ...
                    Attendance::updateOrCreate(
                        ...
                        [
                        'user_id' => auth()->user()->id,
                        'date' => ($Attendance?$Attendance->date:$cur_datetime->setTime(8, 30, 00)->toDateTimeString()),
                        // 'date' => date('Y-m-d'),
                        // 'student_id' => $student_id,
                        'status' => 'Present',
                        ]
