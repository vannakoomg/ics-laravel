...
//	dd('ddd');      
       ...
       // $campus = ['MC'=>'MC','CC'=>'CC'];
	$campus_selected = request()->has('campus')?request()->campus:$campus['MC'];
	$class_selected = request()->has('classes')?request()->classes:'';

        $classes = SchoolClass::where('campus',$campus_selected)->orderBy('name','asc')->pluck('name','id')->prepend('All', '');
        $current_filter = [
		'campus' => $campus_selected,
...
//	dd($campus);
        ...
        return view('admin.users.index', compact('campus','classes','current_filter'));
    }
    ...
    public function ajax_userlist(Request $request){
        ...
        if($request->ajax()){
		    ...
		    })->when($request->has('classes') , function($query) use ($request){
			    if(!empty($request->classes))
				    $query->where('class_id',$request->classes);
			    else
				    $query->where('class_id','!=',69);
		    });
		    ...
		    if($request->role==4)
                ...
                ->editColumn('photo', function(User $user){
                    return '
                       <div style="padding:0;margin:0;width: 100px;max-height:110px;height:100px;overflow:hidden;border:1px solid #d1d1d1" >
                           <img clas="img-fluid img-thmbnail" src="' . "https://storage.googleapis.com/ics_school/photo/".  $user->photo . '" style="width:100%;height:auto">
                       </div>';
                ...
                ->addColumn('voice' , function(User $user){    
                        ...
                        if($user->voice){
                                ...
                                $str .='<source src="' . (Storage::exists('/audio/' . $user->voice)?"https://storage.googleapis.com/ics_school/audio/".$user->voice:'') . '" type="audio/mp3">';
                                $str .='</audio>';
                        }
                        return $str;
                })
