...
class AnnouncementApiController extends Controller
{
    ...
    public function getAnnouncementList(Request $request){
        ...
        $data = Announcement::select('id','user_id','title','view','send','thumbnail','created_at','updated_at')->selectRaw('"" as body')->selectRaw("concat('" . asset('storage/thumbnail') . "','/',thumbnail) as full_image")->where('send',1)->orderBy('created_at','desc')->paginate(10);
    ...
    public function getAnnouncementDetail(Request $request){
        ...
        if(!$request->has('announcement_id'))
           ...
           $data = Announcement::select('id','user_id','title','send','thumbnail','created_at','updated_at','body')->selectRaw("concat('" . asset('storage/image') . "','/',thumbnail) as full_image")->find($request->announcement_id);
