    ...
    $(document).ready(function(){
        ...
        $(document).on('click','.btn_takeout', function(e){
         ...
         // processData: false,
          success: (data) => {
			...
			}else if(data.status=='success_selfout'){
				$('#scanout').removeClass('text-info');
				$('#scanout').removeClass('text-primary');
                                $('#scanout').removeClass('text-danger');
                                $('#scanout').addClass('text-info');
       ...
       // "deferLoading": 0,
        ...
        $.ajax({
			...
			if(data.status=="success" || data.status=='success_selfout'){
			    $('#scanout').removeClass('text-success'); 	
                            $('#scanout').removeClass('text-danger');
			    $('#scanout').removeClass('text-warning');

			    if(data.status=='success_selfout')
			      $('#scanout').addClass('text-success');
		            else
                              $('#scanout').addClass('text-primary');
                            remove_sound(data.student_id);
                            $('#row_' + data.student_id).remove();
                            ...
                            // table4.ajax.reload();
                                ...
                                $.ajax({
                                    type:'POST',
                                    url: 'https://school.ics.edu.kh/admin/calling_dashboard/action_notification/' + rfidno,
                                    data: {},
                                    cache:false,
                                    dataType: 'json',
                                    // contentType: false,
