...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ $user->roles->contains(3) ?'Teacher': ($user->roles->contains(4)?'Student': trans('cruds.user.title_singular')) }}
    </div>
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="email">{{ ( !$user->roles->contains(4) ?'Email':'Student ID') }}</label>
            ...
            @if($user->roles->contains(4) || $user->roles->contains(3))
            
            <div class="form-group">
            <label class="required" for="phone">{{ 'Phone' }}</label>
            <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone',$user->phone) }}" required>
                @if($errors->has('phone'))
                    <div class="invalid-feedback">
                        {{ $errors->first('phone') }}
                    </div>
            ...
            @endif
                ...
                <div class="col-5">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('namekh') ? 'is-invalid' : '' }}" type="text" name="namekh" id="namekh" value="{{ old('namekh', $user->namekh) }}">
                        ...
                        <div class="col-8">
                            <div class="form-group">
                                ...
                                <div class="form-switch">
                                    ...
                                    <label class="form-check-label" for="flexSwitchCheckChecked">{{ 'RFID Card' }}</label>
                                  </div>
                                </label>
                               
                                <input class="form-control {{ $errors->has('rfidcard') ? 'is-invalid' : '' }}" readonly type="text" name="rfidcard" id="rfidcard" value="{{ old('rfidcard',$user->rfidcard) }}">
                ...
                <div class="col-2">
                    <div class="form-group">
                    <img src="{{ ($user->photo ? asset('storage/photo/' . $user->photo ?? '') : asset('storage/image/' . ($user->roles->contains(4) ? 'student-avatar.png' : 'teacher-avatar.png'))) }}" class="img-thumbnail btn btn-outline-primary" id="img_thumbnail" alt="select thumbnail" width="150px" height="150px">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('class') ? 'is-invalid' : '' }}" name="class_id[]" id="class_id" multiple>
                        @foreach($classes as $id => $class)
                            <option value="{{ $id }}" {{ (in_array($id, old('class_id', [])) || $user->classteacher->contains($id))  ? 'selected' : '' }}>{{ $class }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('class'))
                        <div class="invalid-feedback">
            ...
            <div class="col-6">
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('class') ? 'is-invalid' : '' }}" name="course_id[]" id="course_id" multiple>
                        @foreach($courses as $id => $course)
                            <option value="{{ $id }}" {{ (in_array($id, old('course_id', [])) || $user->courseteacher->contains($id))  ? 'selected' : '' }}>{{ $course }}</option>
                        @endforeach
                    </select>
                        ...
                        <div class="col-6">
                            <div class="mb-1 ">
                                 <input type="checkbox" id="is_checkout" name="is_checkout" value="1"
                                    {{ $user->is_checkout ? 'checked' : '' }} class="mr-2">
                                Self-Checkout
                            </div>
                        </div>
            ...
            <div class="row">
                <div class="col-2 text-center">
                    <div class="form-group">
                    <img src="{{ ($user->guardian1 ? asset('storage/photo/' . "{$user->id}_guardian1.png" ?? '') : asset('storage/image/guardian-avatar.png')) }}" class="img-thumbnail btn btn-outline-primary" id="collect_thumbnail1" alt="select thumbnail" width="150px" height="150px">
                    ...
                    <div class="form-group">    
                        <select name="guardian1" id="guardian1" class="form-select" >
                            @foreach ($collection_guardian as $item)
                                <option value="{{ ($item=='Select One Item')?'':$item  }}" {{ ($user->guardian1 ? $user->guardian1: old('guardian1')) == $item ? 'selected' : '' }}>{{  $item }} </option>
                            @endforeach
                        </select>
                ...
                <div class="col-2 text-center">
                    <div class="form-group">
                    <img src="{{ ($user->guardian2 ? asset('storage/photo/' . "{$user->id}_guardian2.png" ?? '') : asset('storage/image/guardian-avatar.png')) }}" class="img-thumbnail btn btn-outline-primary" id="collect_thumbnail2" alt="select thumbnail" width="150px" height="150px">
                    ...
                    <div class="form-group">    
                        <select name="guardian2" id="guardian2" class="form-select" >
                            @foreach ($collection_guardian as $item)
                                <option value="{{ ($item=='Select One Item')?'':$item }}" {{ ($user->guardian2 ? $user->guardian2: old('guardian2')) == $item ? 'selected' : '' }}>{{  $item }} </option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-2 text-center">
                    <div class="form-group">
                    <img src="{{ ($user->guardian3 ? asset('storage/photo/' . "{$user->id}_guardian3.png" ?? '') : asset('storage/image/guardian-avatar.png')) }}" class="img-thumbnail btn btn-outline-primary" id="collect_thumbnail3" alt="select thumbnai" width="150px" height="150px">
                    ...
                    <div class="form-group">    
                        <select name="guardian3" id="guardian3" class="form-select" >
                            @foreach ($collection_guardian as $item)
                                <option value="{{ ($item=='Select One Item')?'':$item }}" {{ ($user->guardian3 ? $user->guardian3: old('guardian3')) == $item ? 'selected' : '' }}>{{  $item }} </option>
                            @endforeach
                        </select>
                    </div>
                </div>
